...
export default Router.prefix("users").group([
...
export default class UserController extends BaseController {
    ...
    public async update(request: Bejibun.Request): Promise<Response> {
        const user = await UserModel.findOrFail(request.input("id")).update({
            name: request.input("name"),
            email: request.input("email")
        });
    ...
    public async destroy(request: Bejibun.Request): Promise<Response> {
        await UserModel.findOrFail(request.input("id")).delete();
