...
```ts
const host = config("database.connections.pgsql.host");
```
...
The first segment of the key names the `config/<file>.ts` module, and the remaining segments walk into its default export.

```ts
config("cache.default");            // config/cache.ts → .default

config("database.connections.pgsql.host");  // config/database.ts → .connections.pgsql.host

config("cors.origin");              // config/cors.ts → .origin
```

Nested values are resolved with dot notation, and `null` is returned when a path doesn't exist.
...
<Note>
    As of v0.6.0, storage ships as the standalone [`@bejibun/storage`](/v0.6/guides/storage/file-storage) package
    (`bun ace install @bejibun/storage`) rather than being built into `@bejibun/core`. The old `config/disk.ts` is
    now `config/storage.ts`.
</Note>
...
<Note>
    As of v0.6.0, the `RateLimiter` facade ships as the standalone
    [`@bejibun/limiter`](/v0.6/guides/security/rate-limiting)
    package. This config file's shape is unchanged, and `RateLimiterMiddleware` (still exported from `@bejibun/core`)
...
Defines the queue connection used by `queue:work` and `queue:retry`,
including how long a worker waits between polls and how long a job stays reserved before another worker can reclaim it.

Queue settings are stored in:
...
| Option        | Description                                                                                                                                                                                  |
| ------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `driver`      | The queue driver for this connection. Currently only `QueueDriverEnum.Database` is supported.                                                                                                |
| `table`       | The table jobs are read from and written to.                                                                                                                                                 |
| `retry_after` | Seconds a worker waits between polls when idle, waits before retrying a failed job, and how long a job stays reserved by a worker before it's considered abandoned and reclaimed by another. |

<Note>
    If `config/queue.ts` isn't present in your application, the package's built-in default (shown above) is used.
</Note>

These settings help control how background jobs are processed and how workers coordinate with each other.
See [Queues & Jobs](/v0.6/guides/background/queues) for how dispatching and worker concurrency work.
...
- Facilitator connection details (defaults to the Coinbase facilitator when omitted)
- Config-wide network/price/payout defaults (EVM and Solana)
- Monetized endpoints
