...
<Note>
    As of v0.6.0, storage ships as the standalone
    [`@bejibun/storage`](/v0.6/guides/storage/file-storage) package (`bun ace install
    @bejibun/storage`) rather than being built into `@bejibun/core`. The old `config/disk.ts` is now
    `config/storage.ts`.
</Note>
...
<Note>
    ...
    [`@bejibun/limiter`](/v0.6/guides/security/rate-limiting) package. This config file's shape is
    unchanged, and `RateLimiterMiddleware` (still exported from `@bejibun/core`) works exactly as
    before.
</Note>
...
<Note>
    If `config/queue.ts` isn't present in your application, the package's built-in default (shown
    above) is used.
</Note>
