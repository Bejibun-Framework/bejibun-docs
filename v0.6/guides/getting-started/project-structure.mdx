...
<Note>
    ...
    @bejibun/storage`), rather than being built into `@bejibun/core`. The old `config/disk.ts` is
...
<Note>
    ...
    [`@bejibun/limiter`](/v0.6/guides/security/rate-limiting) package. `config/limiter.ts`'s shape
    is unchanged, and `RateLimiterMiddleware` (still exported from `@bejibun/core`) works exactly as
    before.
</Note>
