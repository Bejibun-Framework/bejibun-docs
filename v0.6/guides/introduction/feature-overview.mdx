...
<Tip>
    ...
    merging JSON body, route params, query string, and form data -- bracket query keys (like
    `filters[status]=active` or `tags[]=a`) are unwrapped into nested arrays and objects. See [The
    Request](/v0.6/guides/http-layer/the-request) for the full accessor set.
</Tip>
