...
<Tip>
Read request data with `request.input(key?, fallback?)` and validate it with `request.validate(validator)`. The global `RequestMiddleware` populates `request.payload`, merging JSON body, route params, query string, and form data.
</Tip>
