...
Environment variables allow your application to behave differently across development, testing, staging, and production environments without changing source code.

Bejibun uses environment variables to store configuration values that may vary between deployments, such as database credentials, API keys, hostnames, and runtime settings.
...
SQLite is often used for local development and testing.
...
If your application uses email functionality, configuration may look similar to:
...
Mail-related variables should always remain outside source code.
...
These guides explain how Bejibun applications are deployed, how requests flow through the framework, and how services are managed internally.